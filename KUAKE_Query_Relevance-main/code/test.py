...
with open('../data/KUAKE/KUAKE-QQR_test.json', 'r', encoding='UTF-8') as input_data, \
    ...
    for block in json_content:
        ...
        feature=feature[0]
        input_ids = torch.tensor(np.array(feature.input_ids)).unsqueeze(0).cpu()
        attention_mask = torch.tensor(np.array(feature.attention_mask)).unsqueeze(0).cpu()
        token_type_ids = torch.tensor(np.array(feature.token_type_ids)).unsqueeze(0).cpu()
